...
def train(train_loader, val_loader, model, cfg):
    ...
    if cfg.method.name == "leo":
        params_model = [*model.encoder.parameters(), *model.decoder.parameters()]
        params_backbone = [*model.feature.parameters()]
        params_lr    = [model.inner_lr, model.finetuning_lr]

        optimizer_model = instantiate(cfg.optimizer_cls, params=params_model, weight_decay=cfg.method.weight_decay)
        optimizer_backbone = instantiate(cfg.optimizer_cls, params=params_backbone, weight_decay=cfg.method.weight_decay)
        optimizer_lr = instantiate(cfg.optimizer_cls, params=params_lr)
        #optimizers = [optimizer_model, optimizer_lr] # do not optimize backbone (it will remain frozen)
        optimizers = [optimizer_model, optimizer_backbone, optimizer_lr]
    else:
